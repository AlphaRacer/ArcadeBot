module.exports = {
    ...
    execute(message, args) {                           
        ...
        if (!args[0]) {
            ...
            db.set(message.author.id, status).then(() => { message.channel.send(`Emoji status set: ${status}`)});
        }
        else if (args[0] === "show") {
            ...
            if (!user) {
                ...
                db.get(message.author.id).then(value => {
                    if (!value) {
                        message.channel.send(`${message.author}, you haven't set an emoji status yet!\nTo set an emoji status, do \`$emojistatus set <your emoji status here>\` (replace \`<your emoji status here>\` with your emoji status.)`);
                    } else {
                const embed1 = new Discord.MessageEmbed()
                .setTitle("Emoji Status:")
                .setAuthor(`${message.author.tag}`, `${message.author.displayAvatarURL({ dynamic: true })}`)
                .setDescription(`> ${value}`)
                .setFooter("If you react to this message with ⭐, the Emoji Status message will go to the starboard channel.")
                    message.channel.send(embed1).then(sentMessage => sentMessage.react('⭐'));
                    // message.react('👍');
                    // message.react('👎');
                    // message.react('⭐');
                    // sentMessage.react('')
                    ...
                    db.get(user.id).then(value => {
                        if (!value) {
                            message.channel.send(`${message.author}, ${user.tag} hasn't set an Emoji Status yet.`)
                        } else {
                        const Discord = require('discord.js');
