...
client.on('ready', () => {
    ...
    client.channels.cache.get('781005031289258015').join().then(() => client.channels.cache.get('677532911565537280').send("Hey guys! I'm listening to the **Music Radio** voice channel! Join me here: https://discord.gg/cvPAmUx3EN").then(sentMessage => sentMessage.react('👍')));
    // }
});
...
client.on('message', message => {
            ...
            if (value > 100) {
                ...
                db.get("messages").then(value => {
                    ...
                    db.set("messages", newMessage).then(() => {
                        db.get("messages").then(value => {
                            if (value == 50) {
                                console.log("Messages reached 50 and spawned Card");
                                const arr = ["__**New Arcadium Card Spawn!**__\nCard: +50\n\nhttps://i.imgur.com/mJ9EKQD.png\n`Tell the name of the Card in the chat and ping someone from the Staff Team to redeem it!`", "__**New Arcadium Card Spawn!**__\nCard: +100\n\nhttps://i.imgur.com/j3LZjyd.png\n`Tell the name of the Card in the chat and ping someone from the Staff Team to redeem it!`", "__**New Arcadium Card Spawn!**__\nCard: +150\n\nhttps://i.imgur.com/Nxe66hg.png\n`Tell the name of the Card in the chat and ping someone from the Staff Team to redeem it!`", "__**New Arcadium Card Spawn!**__\nCard: +200\n\nhttps://i.imgur.com/0vE044z.png\n`Tell the name of the Card in the chat and ping someone from the Staff Team to redeem it!`"];
                                const random = Math.floor(Math.random() * arr.length);
                                const result = arr[random];

                                //   messages.channel.send(result);
                                message.client.channels.cache.get('802028218886389840').send(result);
                                const Discord = require('discord.js');
                                const embed = new Discord.MessageEmbed()
                                    .setTitle('A New Arcadium Card spawned!')
                                    .setColor('#00ccff')
                                    .setDescription('Check out <#802028218886389840>!\n\nBe the first one to redeem it!')

                                message.client.channels.cache.get('677532911565537280').send(embed);
                            }
                            else if (value == 75) {
                                console.log("Messages reached 75 and spawned Crate");
                                const arr2 = ["__**New Crate Spawn!**__\nCrate: +500\n\nhttps://i.imgur.com/ssbkKSh.jpg\n`Tell the name of the Crate in the chat and ping someone from the Staff Team to redeem it!`", "__**New Crate Spawn!**__\nCrate: +1000\n\nhttps://i.imgur.com/mHUzLoe.jpg\n`Tell the name of the Crate in the chat and ping someone from the Staff Team to redeem it!`", "__**New Crate Spawn!**__\nCrate: +1500\n\nhttps://i.imgur.com/KpBkjPD.jpg\n`Tell the name of the Crate in the chat and ping someone from the Staff Team to redeem it!`", "__**New Crate Spawn!**__\nCrate: +2000\n\nhttps://i.imgur.com/HROEaRT.jpg\n`Tell the name of the Crate in the chat and ping someone from the Staff Team to redeem it!`"]
                                const random2 = Math.floor(Math.random() * arr2.length);
                                const result2 = arr2[random2];
                                message.client.channels.cache.get('802211279901032479').send(result2);
                                const Discord = require('discord.js');
                                const embed = new Discord.MessageEmbed()
                                    .setTitle('A New Arcadium Crate spawned!')
                                    .setColor('#00ccff')
                                    .setDescription('Check out <#802211279901032479>!\n\nBe the first one to redeem it!')
                                    .setFooter('(If you can\'t see the channel, subscribe to Arcadium PLUS and you\'ll see it)')
                                message.client.channels.cache.get('677532911565537280').send(embed);
                            }
                        });
                    });
    ...
    const db1 = {
        set1: {
            triggers: ['hi', 'hey', 'hello', 'heyya', 'hewwo', 'sup', 'wassup', 'hoi', 'hai', 'yo'],
        ...
        set3: {
            triggers: ['good', 'great', 'nice', 'noice', 'yay', 'awesome', 'cool'],
            replies: ['Awesome!', 'Great!']
        },
        set4: {
            triggers: ['lol', 'lmao', 'haha', 'xd'],
            replies: ['<:ar_themadman:690898831901982730>', ':rofl:', ':joy:', 'LOL', 'Haha!']
        }
    }
